...
**Gated behind `@media screen and (min-width: 800px)` since 2026-09-24.** Shipped without a media query, and the user then reported from the live site that below 800px wide the rule wrecks Instagram's default modal -- seen in a narrow window on a desktop computer, and on a phone sending a desktop user agent with desktop mode OFF, so with the phone's real narrow viewport -- so it now lives in the block's second 800px group, beside the slide-counter re-anchor. The breakpoint is the one the rest of this block already uses; that it coincides with where Instagram's modal changes layout is the user's observation, not a measured threshold. **Confirmed live on 2026-09-24:** below 800px the post and reel modals both keep Instagram's stock caption column and ignore the setting, and on the phone with desktop mode ON the setting still drives the column -- so the case that motivated the rule sits above the gate.
