...
KNOWN GAP: the stacked state. When the caption column cannot hold `--u-caption-min` it wraps onto its own line at full width, and then the host is neither beside nor below the media in the way either rule assumes -- measured at `--u-media-column` 80 and 100, where the host spans the full row and its top no longer matches the media's. The badge is misplaced there. It is a narrow band: it needs a wide media column or a window narrow enough to squeeze the caption, while still being 800px or wider. Not fixed, because no single rule covers both states and CSS cannot ask whether a flex line wrapped.
