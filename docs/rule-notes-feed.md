...
**2026-09-14: the shape this actually costs is 3:4, not 4:5.** The user wrote the original limitation while taking 3:4 for 4:5, and has since seen a creator posting consistently in 3:4; no genuinely 4:5 video has ever been confirmed. It changes none of the reasoning above -- 3:4 is flattened into the same 125% box, so it is indistinguishable from 9:16 in exactly the same way -- but it makes the cost concrete rather than hypothetical. Past the knee the box is a true 9:16, so a 3:4 source loses `1 - 1.3333 / 1.7778` = 25% of its width; against the box measured above, 550x858.8, it scales to 644.1 wide to cover the box and loses 94.1px, about 15%. So the crop is present at the defaults and the setting decides its size. Per that note, the 858.8 was taken when `--u-feed-width` defaulted to 80 rather than today's 90, so treat the 15% as approximate. `object-fit: contain` on the box's video would trade that crop for banding on both shapes without needing to tell them apart; it was put to the user with these numbers on 2026-09-14 and declined.
