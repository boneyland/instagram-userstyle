...
Anchoring to the property name and the whole value fixes both halves at once, and the trailing `%` stops it swallowing `125.5%`. Matching the whole value is safe here, and is the deliberate exception to the standing rule against it, because `125%` is not a computed ratio: it is the constant Instagram clamps anything taller than 4:5 to. Across all sixteen snapshots the round values occur only as the bare `75%`, `100%` and `125%`; every computed value carries decimals. Verified on the landscape reel's box in `Instagram_feed2.html` by substituting values and reading back which selectors matched: `68.125%`, `56.2696%`, `75%`, `100%` and `125.5%` all classify landscape, `125%` in both spacings classifies portrait.
