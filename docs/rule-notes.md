...
The selector is the feed-column rule below lifted one level, to that column's parent. It has the same reach -- 1 match on each saved feed, 0 on all fifteen other snapshots -- so the reservation cannot leak onto a post, reel or modal page.
...
Anchoring to the property name and the whole value fixes both halves at once, and the trailing `%` stops it swallowing `125.5%`. Matching the whole value is safe here, and is the deliberate exception to the standing rule against it, because `125%` is not a computed ratio: it is the constant Instagram clamps anything taller than 4:5 to. Across all seventeen snapshots the round values occur only as the bare `75%`, `100%` and `125%`; every computed value carries decimals -- the 4:3 carousel added on 2026-09-12 carries `75%` and `74.9712%` side by side on the same page, which is the pattern in miniature. Verified on the landscape reel's box in `Instagram_feed2.html` by substituting values and reading back which selectors matched: `68.125%`, `56.2696%`, `75%`, `100%` and `125.5%` all classify landscape, `125%` in both spacings classifies portrait.
...
**On the feed it matches a photo**, which is the concrete reason this rule is scoped by URL rather than trusted to its own guard. `Instagram_feed2.html` article 10 is a sponsored single photo in a `padding-bottom:177.8%` box — alt text "Photo by …", no `<video>` and no `/reels/` link anywhere in the article — and `main div[style*="padding-bottom"][style*="177"]` matches it. Unscoped, this rule would collapse that photo's box and force it to `min(95vh, 950px)`. So a 9:16 photo is a shape Instagram does accept; it is simply not one any post-page snapshot holds.

Only 177.778% has ever been captured on a post or reel page, so every other reel ratio keeps Instagram's box there. A census of every inline `padding-bottom` box across all seventeen snapshots found no `<video>` in any 133% box on any page: **no 3:4 reel has ever been captured**, on a post page or on the feed. On the feed it could not be — Instagram flattens anything taller than 4:5 to 125%, which is the note further up this file — so a 3:4 reel can only exist as a post-page shape, and no capture of one exists to say what box Instagram writes for it.


## `@-moz-document regexp("https://www\\.instagram\\.com/(p|[^/]+/p|[^/]+/reel)/.*")` (second block)

Post permalink pages with URL https://www.instagram.com/p/<id>/ and https://www.instagram.com/<user>/p/<id>/ — and, since 2026-09-12, https://www.instagram.com/<user>/reel/<id>/ as well.

The reel path was added because **every post shape is reachable at that URL, not only reels** -- single photos and carousels included. That is the user's live observation, reported 2026-09-12; how a non-reel comes to be served at a `/reel/` URL was not established, and there is no snapshot of one to confirm any of it here. What is measured is that the widening costs nothing on the reels that are captured: on all three reel snapshots every selector in this block matches 0 elements, and `scoped.py --diff` against the previous version reports 0 standard and 0 custom property changes on all seventeen snapshots, with `Instagram-user-reel-id.html` moving from 2 applicable blocks to 3.

The consequence to keep in mind is that this block's condition is now **character-for-character identical to the reel block's above** — `scoped.py --urls` prints them as two identical rows. Nothing separates the two blocks but their selector guards, so a guard loosened in either one is now loosened against the same set of pages. They could be merged into one block; they are kept apart because the guards, not the URLs, are what the notes above and below are written about.
