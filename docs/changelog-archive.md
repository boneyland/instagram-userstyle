...
Releases before `2026.9.12.1`, moved out of `CHANGELOG.md` to keep the current entries readable. Nothing here is edited — these are the entries as they were written, each comparing itself against the version published before it.

**The deletions and deliberate omissions from these releases are not here.** They are in `docs/removed.md`, organized by subject, because a "why this is not in the style" record is needed long after the release that made it and is the thing most likely to be re-added by mistake. The entries below keep a one-line pointer where such a section used to be.

For the current releases, see `CHANGELOG.md`.
...
- **The post-page carousel `zoom` rule and its setting, "Post page: enlarge a multi-photo post by" (`u-post-media-scale`, default 1.5).** Both are gone. The rule did nothing visible, measured live on 2026-09-11 on both post URLs. Why it never worked, why every offline measurement said otherwise, and why the feed's `zoom` rule was checked the same day and **kept**, are all in `docs/removed.md`.
...
- **No zoom rule for the square carousel**, though one was asked for — and none is needed: Instagram re-derives the slide width from the container, confirmed live on 2026-09-11. Capping the container is the whole job. The measurements, the rejected `tan(atan2())` version, and the saved-page overflow artifact are in `docs/removed.md`.
...
- **`@description` no longer states the browser requirement.** The floor is unchanged at Firefox 126; it is stated in `README.md` and `USw-notes.md` instead. Do not re-add it to `@description`. See `docs/removed.md`.
...
- **Every comment outside the metadata block.** The style went from 367 lines to 232; no selector, declaration or block condition was touched, and the metadata header is byte-for-byte identical. The equivalence proof and the reasoning are in `docs/removed.md`. `docs/rule-notes.md` is now the canonical record — **before loosening a guard or changing a number, read the note there.** Nothing in the style will warn you any more.

### Not done

- **No height cap.** See `docs/removed.md` — there is nothing for a cap to do yet, and `max-height` on a `padding-bottom` box would not be the way to build one.
...
- **No guard was written against a *mixed* carousel** — photo plus video slides — on a `/p/` URL.
- **Carousels are still uncapped, deliberately.**

Both in `docs/removed.md`, with what to try first and why it was left alone.
...
- **The collapsing left nav**, **the settings-page padding rules**, and **`svg[aria-label="Instagram"]`** — three nav rules and their leftovers.
- **The `:root` half of the `--system-14-*` override.**
- **`:not(.xtcbf50)`** from the two-column post selector.

Each with its evidence in `docs/removed.md`. The `:not(.xtcbf50)` entry is the one no offline check can validate — restore it first if a post type ever lays out wrongly.
...
A report of high memory use on some Instagram pages, in both Firefox and Chromium. **No root cause was found and nothing was changed.** Three hypotheses were tested and all three came back negative. The full record, and the measurement that would be needed to beat the null explanation, is in `docs/removed.md` under "High memory use on some Instagram pages" — do not re-open them without a reproduction.
