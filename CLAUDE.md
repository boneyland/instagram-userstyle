...
The snapshots themselves — **19 captures across 16 rows as of 2026-09-17**, and what each one is evidence *for* — are in `docs/snapshots.md`. The folder was re-captured that day, so counts written into older notes and changelog entries describe a corpus of 21-22 that no longer exists; that file dates the change and lists which captures carry Instagram's new `x1ykew4q` wrapper. Read it when you need the capture that exercises a rule you are about to change. Three earn a mention here: `Instagram_feed3.html` is the newest feed and therefore **the one `verify.py` now loads**, `Instagram_feed2.html` is the richest and holds the only mixed carousel and the only 9:16 single photo, and `Instagram-p-id-modal(single_photo).html` is the only saved page with an `<article>` inside a `[role="dialog"]`, which makes it the test for every modal guard.
