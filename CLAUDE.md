...
| `regexp(.../(p\|[^/]+/p\|[^/]+/reel)/...)` | **carousels and single photos** on all three of those paths | the carousel's `li[style*="translateX"]`; for a single photo, a photo box with neither that nor a `video` |

The last two blocks now carry the **same condition, character for character** — `scoped.py --urls` prints them as two identical rows — so all three blocks claim every post URL and the reel URL alike, and it is their *selector* guards, video versus slide list versus bare photo box, that decide which owns a given post. Three post shapes share three URL patterns; every rule stays in its lane by selector. The reel path was added to the second block on 2026-09-12 because **every post shape is reachable at `/<user>/reel/<id>/`, not just reels** — the user's live observation; no snapshot of a non-reel at that URL exists. It is inert on the three reel snapshots that do: block-3 selectors match 0 elements on each, and `scoped.py --diff` reports 0 standard and 0 custom property changes across all seventeen, with `Instagram-user-reel-id.html` going from 2 blocks to 3.

**Square** media is the shape whose lever is **not** `--x-maxWidth`, and this is true of a square carousel and a square single photo alike. **Landscape is the same** — `Instagram-p-id(carousel75).html` and `Instagram-p-id(single_photo75).html` both carry no token either, so the no-token rule is "every carousel Instagram did not give `.xf68679`", not "square carousels". Instagram omits the class that reads that token on square and landscape photos — both measured, no longer inference — so the rule sets `max-width` on the column directly, which works on every single-photo layout. `.xvc5jky` in that selector is load-bearing: without it the guards also match the "more posts" grid below the post.
...
- **On the POST and REEL pages, reels are only handled at 9:16.** Every reel snapshot of those pages is `padding-bottom:177.778%`, and the rule that collapses that box matches the literal `177`. A reel of any other ratio keeps Instagram's box on all three reel URLs. Same limitation the reel pages always had; it is not new to the post-permalink paths. **No snapshot holds a reel of any other ratio at all** — a census of every inline `padding-bottom` box across all seventeen found no `video` in any 133% box on any page, so a 3:4 reel has never been captured anywhere, on a post page or on the feed. On the feed it could not be: Instagram flattens anything taller than 4:5 to 125%. The **feed** is no longer in this position — since 2026.9.11.7 a non-125% feed reel is sized from the video's intrinsic dimensions instead, which needs no ratio matching at all, and was confirmed live on 2026-09-11 at 75% and 56.25%. See the note in `docs/rule-notes.md` before touching it: that rule cannot be measured offline at all.
