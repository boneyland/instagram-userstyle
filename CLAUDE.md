...
**It is blind to URL scoping, and to two other things.** `prepare()` flattens every `@-moz-document` block unconditionally — which is exactly what makes the two-file mode a valid equivalence proof — so a change to a block's *condition* produces no diff at all. It also only ever loads the feed snapshot (`find_snapshot()` matches `Instagram (*).html` inside `Instagram_snapshots/`, and that name alone), so nothing it says covers the reel, post or modal pages. A passing run therefore means "the rules still parse and still do the same thing on the feed", never "this change is correct".
...
**Browser floor is Firefox 126** — `:has()` needs 121, `zoom` needs 126. Going higher means updating `@description`, which states the requirement.
...
**The style carries no comments outside the metadata block.** Every one was stripped on 2026-09-11 — see the `Removed` entry under 2026.9.11 in `CHANGELOG.md`, which records the equivalence proof. **Do not reintroduce them.** `docs/rule-notes.md` is now the only explanation of why a rule is shaped the way it is, so when you change a rule, the note there is not a duplicate to keep in sync — it is the record. Write it.
...
- **No Chromium floor has been established.** Firefox 126 is the only floor stated anywhere. The style is known to work in one ungoogled-chromium build (2026-09-10) and nothing narrower has been checked. Both `@description` and `USw-notes.md` name Firefox only, so a Chromium user gets no guidance at all — decide what to claim before advertising Chromium support.
...
- The post-page defaults are calibrated against **one aspect ratio**. Both post-page carousel snapshots hold a 3:4 carousel whose media is 449px wide, and the 1150px column default comes from that (caption 429 + 449 × 1.5). A squarer or landscape post starts wider and will want a wider column at the same scale. The two settings are coupled and cannot be coupled automatically — CSS cannot read the media's own width to derive one from the other.
- **Reels are only handled at 9:16.** Every reel snapshot is `padding-bottom:177.778%`, and the rule that collapses that box matches the literal `177`. A reel of any other ratio — the feed documents 4:3 reels at 75% — keeps Instagram's box on all three reel URLs. Same limitation the reel pages always had; it is not new to the post-permalink paths.
