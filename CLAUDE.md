...
| `Instagram-20260912.4-uploaded.user.css` | The currently published version, `2026.9.12.4`. Diff the deliverable against this one, and write each new `CHANGELOG.md` entry and the whole of `USw-notes.md` as a comparison with it — **do not edit**. |
| `Instagram-20260912.1-uploaded.user.css`, `Instagram-20260910-uploaded.user.css`, `Instagram-20260907-uploaded.user.css` | Previously published versions. Kept for history — **do not edit**. |

There is one uploaded copy per publication and **the newest is always the baseline — check, do not assume.** The local files cannot settle which version is actually live, only which were uploaded at some point: read the version off [the style's page on USw](https://userstyles.world/style/30052/instagram-desktop-site) and match it to a copy here. Ask rather than inferring if the page is unreachable. The filename mirrors `@version`: `Instagram-YYYYMMDD-uploaded.user.css` while a day holds one release, `Instagram-YYYYMMDD.R-uploaded.user.css` once it holds two, which is why `2026.9.12` has a `.1` and a `.4` and the two earlier days have no suffix. Earlier copies are not renamed when the scheme changes. Do not pick the baseline by sorting the filenames: `-` sorts before `.`, and a two-digit revision would sort before `.4`. Read the `@version` out of the candidates instead.
...
The snapshots themselves — twenty-one captures across fourteen rows, and what each one is evidence *for* — are in `docs/snapshots.md`. Read it when you need the capture that exercises a rule you are about to change. Two earn a mention here: `Instagram_feed2.html` is the richest feed and the one `verify.py` loads, and `Instagram-p-id-modal(single_photo).html` is the only saved page with an `<article>` inside a `[role="dialog"]`, which makes it the test for every modal guard.
...
A mixed carousel (photo plus video slides) whose column has no token now matches **two** rules — the slide-list cap and that new `:has(video)` cap. They set the same property to the same value, so it is harmless; it is the one place in the block where two rules deliberately overlap. See `docs/removed.md`.
