...
- **The post modal setting no longer breaks narrow windows.** Below 800px wide — a narrow desktop window, or a phone browser with desktop mode off — the caption column setting squeezed Instagram's own modal out of shape. It now applies only from 800px up, and below that the modal is left as Instagram draws it. A phone browser in desktop mode still takes the setting. Nothing else changes.
